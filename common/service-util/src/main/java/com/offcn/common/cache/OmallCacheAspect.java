...
public class OmallCacheAspect {
    ...
    //  切OmallCache注解
    @SneakyThrows
    @Around("@annotation(com.offcn.common.cache.OmallCache)")
    public Object cacheAroundAdvice(ProceedingJoinPoint joinPoint){
        ...
        OmallCache OmallCache = signature.getMethod().getAnnotation(OmallCache.class);

        //  获取到注解上的前缀
        String prefix = OmallCache.prefix(); // sku
            ...
            if (object==null){
                ...
                if (result){
                    try {
                        ...
                        if (object==null){
                            //防止缓存击穿创建一个空对象
                            Object object1= signature.getReturnType().newInstance();
